...
等边三角形的高 = (√3/2) × 边长 = (√3/2) × r。

两个交点之间的距离 = 高度的两倍 = **r√3**。

就这样。两个等圆的最简单重叠，自然涌现出了 √3——一个无法用任何分数表达的无理数。

> **√3 不是被"计算"出来的。它是被"画"出来的。**
...
- **√2**：画一个正方形，它的对角线就是边长的 √2 倍。而正方形可以从 Vesica Piscis 中直接提取——两个圆的公共弦就是正方形对角线的天然起点。

- **√5**：画一个长宽比为 1:2 的矩形，它的对角线就是 √5。

- **黄金比例 φ**：有了 √5，黄金比例就水到渠成—— **φ = (1+√5)/2 ≈ 1.618...**
...
但想想看——毕达哥拉斯的学生希帕索斯因为发现 √2 是无理数而被同门扔进了大海（传说中）。古希腊人为"不可公度量"（无法用分数表达的长度）困扰了几百年。
...
*系列目录：*
1. ***两个圆相遇的地方** — 鱼形囊、√3，以及一朵开遍世界的花*（本篇）

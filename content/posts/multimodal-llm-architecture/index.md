...
上一篇 [《当数字学会了远近亲疏》](/ai-blog/posts/embedding/) 走到了一个惊人的结论：

> 文字、图片、声音——不同的入口，同一个空间。"猫"这个概念，无论你写它、拍它、还是听它叫——在足够好的 Embedding 中，它们都是邻居。

CLIP 把文字和图片映射到了同一个向量空间。ImageBind 把六种模态绑在了一起。柏拉图表示假说甚至暗示：所有模型都在趋向同一个底层现实的表示。
...
这个过程在 [Embedding 那篇文章](/ai-blog/posts/embedding/) 里已经讲透了：**本质上就是查一张训练出来的表。**
...
**这就是在 [Embedding 文章](/ai-blog/posts/embedding/) 中提到的"让文字和图片住进同一个空间"的具体实现方式。**
...
在 [Embedding 文章](/ai-blog/posts/embedding/) 的结尾，我们提到了柏拉图表示假说：
